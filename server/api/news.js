...
module.exports = function (router) {
    ...
    router.get('/news/:id', details);
    router.delete('/news/:id', _delete);
}
...
function add(req, res){
    ...
    db.query("INSERT INTO news SET ?", req.body , (err, result) => {
        if (!err) {
...
async function list(req ,res ){
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM news WHERE CONCAT(title) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
        ...
        db.query("SELECT * FROM news LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
            if (!err) {
                return _response.apiSuccess(res, result.length+" "+responsemsg.found , result , {page: parseInt(page) , limit: parseInt(limit),totalDocs: totalDocs })
    ...
    if (req.params.id){
        db.query("SELECT * FROM `news` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {
                ...
                db.query("UPDATE news SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
