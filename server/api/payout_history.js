...
module.exports = function (router) {
    router.get('/payouts_history', list);
    router.post('/payouts_history', add);
    router.put('/payouts_history/:id', update);
    router.get('/payouts_history/:id', details);
    router.delete('/payouts_history/:id', _delete);
}
...
function add(req, res){
    ...
    db.query("INSERT INTO payouts_history SET ?", req.body , (err, result) => {
        if (!err) {
...
async function list(req ,res ){
    ...
    db.query("SELECT COUNT(*) AS total FROM payouts_history", (err, result) => {
        if (!err) {
    ...
    if (req.query.search_string && req.query.search_string !== ''){
        ...
        db.query("SELECT * FROM payouts_history WHERE CONCAT(title) REGEXP '"+req.query.search_string+"'  LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
        ...
        db.query("SELECT * FROM payouts_history LIMIT "+limit+" OFFSET "+offset+" ", (err, result) => {
            if (!err) {
                return _response.apiSuccess(res, result.length+" "+responsemsg.found , result , {page: parseInt(page) , limit: parseInt(limit),totalDocs: totalDocs })
...
function update(req ,res ){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `payouts_history` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!err && result.length > 0) {

                db.query("UPDATE payouts_history SET ? WHERE id = '"+req.params.id+"'" , req.body ,(err , result) =>{
                    if (!err){
    ...
    if (req.params.id){
        db.query("SELECT * FROM `payouts_history` WHERE id='"+req.params.id+"'", (err, result) => {
            if (!result.length){
                ...
                db.query("DELETE FROM `payouts_history` WHERE id='" + req.params.id + "'", (err, result) => {
                    if (!err) {
